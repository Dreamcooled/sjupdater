...
namespace SjUpdater
{
    public static class SjInfo
    {
        ...
        private static List<DownloadData> ParseSite(ShowData showData, string url, out string nextpageurl, out string firstcover, UploadCache uploadCache)
        {
            ...
            WebResponse respSCP = null;
        
            for (int i = 0; i <= 7; i++)
            {
                try
                {
                    HttpWebRequest reqSCP = WebRequest.CreateHttp(url);
                    reqSCP.AutomaticDecompression = DecompressionMethods.GZip | DecompressionMethods.Deflate;
                    reqSCP.KeepAlive = false;
                    respSCP = reqSCP.GetResponse();
                    break;
                }
            ...
            HtmlDocument docSCP = new HtmlDocument();
            docSCP.OptionDefaultStreamEncoding=Encoding.UTF8;
            docSCP.Load(respSCP.GetResponseStream());
            respSCP.Dispose();

            List<DownloadData> list = new List<DownloadData>();

            HtmlNode contentScb = docSCP.GetElementbyId("scb");
            var dlSeasonst = contentScb.SelectSingleNode("div[@class='bkname']");
            var seasonlinks = dlSeasonst.SelectNodes("a[@href][following-sibling::text()]");
            ...
            for (int i = 0; i <= 7; i++)
            {
                try
                {
                    HttpWebRequest req = WebRequest.CreateHttp(seasonlink.GetAttributeValue("href", null));
